...
const ContasPagar: React.FC = () => {
  ...
  const kpis = useMemo(() => {
    ...
    const mesesComDados = new Set(contasFiltradas.map((c) => c.data_emissao.slice(0, 7))).size;
    const mediaMensal = mesesComDados > 0 ? (totalAberto + totalPago) / mesesComDados : 0;

    return { totalAberto, totalPago, contasVencidas, aVencer30, mediaMensal };
  }, [contasFiltradas]);
