...
describe("presets de período", () => {
  ...
  it("DEFEITO PRESERVADO: na virada do dia, o início é local e o fim é UTC", () => {
    // 01/09 às 02h em Greenwich ainda é 31/08 em Brasília. O início sai de
    // `getFullYear`/`getMonth` (local) e o fim de `toISOString` (UTC), então
    // em Brasília o "mês atual" atravessa a virada: 01/08 a 01/09.
    ...
    expect(periodoDoPreset("mesAtual", viradaDoMes)).toEqual({
      inicio: foraDoUtc ? "2026-08-01" : "2026-09-01",
      fim: "2026-09-01",
    });
  });
});
